...
def hand_ypr(ypr="yaw", handNumber="1"):
	frame = controller.frame()
	if ypr == "yaw":
		return frame.hands[int(handNumber) - 1].direction.yaw * Leap.RAD_TO_DEG
	elif ypr == "pitch":
		return frame.hands[int(handNumber) - 1].direction.pitch * Leap.RAD_TO_DEG
	else:
		return frame.hands[int(handNumber) - 1].palm_normal.roll * Leap.RAD_TO_DEG
